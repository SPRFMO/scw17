...
params <- list(model_name = "h1_0.14", stock = 1)
...
The current model `0.14` represents the short stock-recruit time series and low-productivity case. Three additional runs were added to separate the effects of stock-recruit fitting period and steepness: low steepness with the long time series, high steepness with the long time series, and high steepness with the short time series. Across these runs, $F_{MSY}$ spans `r round(min(productivity_reference$Fmsy, na.rm = TRUE), 3)` to `r round(max(productivity_reference$Fmsy, na.rm = TRUE), 3)`, while $F_{35\%}$ spans only `r round(min(productivity_reference$F35_est, na.rm = TRUE), 3)` to `r round(max(productivity_reference$F35_est, na.rm = TRUE), 3)`.
...
The base run is `h1_0.14`, which uses the short SRR fitting period and low steepness. In this paper, "low productivity" refers to steepness $h = 0.65$ and "high productivity" refers to steepness $h = 0.85$. The short SRR fitting period uses 2000-2022 and the long SRR fitting period uses 1970-2022. The three additional runs are:

1. `h1_0.14ll`: low steepness and long SRR fitting period;
2. `h1_0.14hl`: high steepness and long SRR fitting period;
...
The F profile from `h1_0.14` provides the starting point for comparing MSY-based and SPR-based reference points. In @fig-f-profile, $F_{35\%}$ is marked against the same profile as $F_{MSY}$. The two values are close only under some productivity assumptions. Across all four runs, $F_{35\%}$ is much more stable than $F_{MSY}$ because it does not use the SRR.

```{r}
#| label: tbl-spr-reference
#| tbl-cap: "SPR reference point from model 0.14. SBF35 is interpolated from the yield profile at SPR = 0.35."
...
```{r}
#| label: fig-f-profile
#| fig-cap: "F profile from the 0.14 yield-profile output. Vertical lines mark F35% and Fmsy on the profile."
...
The dynamic-Bzero comparison for `h1_0.14` is shown in @fig-ssb-fished-dynamic-bzero. This is a counterfactual SSB comparison: the no-fishing trajectory is not a fixed scalar but an annual dynamic series.
...
#| fig-cap: "Spawning biomass depletion ratio estimated as fished spawning biomass divided by dynamic Bzero spawning biomass, compared with the reported SSB_NoFishR estimate."

ssb_comparison |>
  filter(panel == "Depletion ratio") |>
  ggplot(aes(year, value, color = source, linetype = source)) +
  geom_line(linewidth = 0.85) +
  labs(x = NULL, y = "SSB / dynamic Bzero", color = NULL, linetype = NULL) +
  theme_report()
```
