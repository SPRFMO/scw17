...
The same productivity suffixes are also summarized for the matching two-stock `h2_0.16` runs: `h2_0.16ll`, `h2_0.16ls`, `h2_0.16hl`, and `h2_0.16hs`. For these h2 runs, stock-specific MSY terms are read by occurrence in the `.std` file, and stock-specific $F_{35\%}$ values are read from the corresponding `_1_R.rep` and `_2_R.rep` files.
...
The `h2_0.16` run set applies the same productivity suffixes to the two-stock model. Because h2 reports stock-specific reference-point quantities, Stock 1 and Stock 2 (Peruvian zone) are shown separately below rather than mixed in the same table and figure.
