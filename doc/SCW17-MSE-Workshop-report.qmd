...
1. This draft was developed from Day 1 and Day 2 meeting notes, the SCW17 working documents, and the daily summary documents. Artificial-intelligence tools were used to help convert notes into report text; all content should be reviewed by the report authors and meeting participants before submission.
...
1. The Chair reviewed the five-day work plan. The group agreed that work would proceed by consensus. A draft agenda item referring to a Thursday "vote" should be revised to "collaboratively agree", because a formal vote would require a quorum and was not the intended decision process.
...
1. The main objectives for the week were to:
   ...
   - Record decisions, unresolved issues, and follow-up tasks for the workshop report to SC14.
...
1. A differentiable penalty was added to discourage mean fishing mortality over ages 2-5 from exceeding high values in implausible solutions. The penalty was described as a regularizing device to help the solver and was not expected to affect converged estimates of biomass or recruitment.
...
1. Participants noted that the differences among model variants were driven substantially by the revised Peruvian inputs and effort-creep treatment. Critiques of model 0.16 were recorded as concerns about its characteristics, not as a positive conclusion that model 0.00 was biologically superior.
...
1. The group discussed how to simulate future observations for indices that may not track stock biomass consistently through time. This is central to the availability and El Nino robustness issues raised at SCW15.

1. Participants agreed that exact simulation of GLM, GAMM, or spatio-temporal standardization procedures is generally infeasible in forward projections. A simpler representation, such as autocorrelation plus observation error, may be more practical. Routine checks should compare simulated index behaviour with historical variability and observed index-stock correlations.

1. On combining multiple indices in empirical rules, weighting by catch was not supported by some participants. The group agreed that weighting by representativity was more appropriate.

## Operating-Model Reference Set

1. The composition of the OM reference set was debated. Including model 0.00 would add an optimistic high-biomass, low-exploitation outcome that could affect CMP tuning and communication. No working document proposing alternative data weighting was tabled during Day 1.
...
1. The Day 1 decisions and agreements were to revise the agenda wording so the group would "collaboratively agree" rather than vote; use model 0.16 as the primary reference for OM conditioning during the workshop; not treat model 0.00 as a base OM on Day 1, while retaining it and the SC13-era configuration as labelled alternatives or robustness tests; use the reduced-parameter, block-based Peruvian fleet selectivity formulation; accept the differentiable high-F penalty as a solver aid that is not expected to change biomass or recruitment estimates; keep the current data-simulation procedure at this stage; weight combined indices by representativity rather than catch; retain Kobe plots for stakeholder communication and add F~35%-based comparisons across OMs; tune CMPs across the reference set rather than to individual OMs; and submit the benchmark report, final Paper 01, and this workshop report to the Scientific Committee.
...
1. Issues identified for further work during the remainder of SCW17 included the final composition and weighting of the OM reference set; the relative influence of Peruvian CPUE changes, effort-creep treatment, and alternative biological interpretations on model 0.16; how to simulate standardized CPUE and other indices in forward projections; whether structurally divergent OMs should be tuned jointly or treated separately; instability in dynamic reference points and apical-F metrics; the choice of running-average window for reference-point calculations; the effect of including optimistic alternatives on probability-of-green-zone tuning and Commission communication; and the operational definition and response process for exceptional circumstances.

# Operating model details

1. Day 2 addressed future recruitment and index simulation, ENSO and broader climate effects, the composition and naming of the OM set, reference-point proxies, and projection-scenario definitions. The day continued directly from the Day 1 discussion of OM conditioning and management-procedure tuning.

## Recruitment Deviations and Index Correlation

1. The group discussed using cross-correlation among indices to simulate future recruitment deviations. Participants noted that historical correlations are not estimated exactly because the series include changes in catchability and selectivity, whereas the projection settings use the more recent period. Residual autocorrelation should also be represented where possible.
...
1. The group discussed the Kobe plot and noted that much of the apparent movement in the plot reflects changes in the reference points rather than changes in the underlying trajectory. F~MSY~ is highly sensitive to steepness because the yield curve is shallow, while B~MSY~ appears more stable.
...
- The Working Group will resolve how the northern stock can be handled when F~MSY~ is not available inside the CMP.
...
1. Issues identified for follow-up included the approximate nature of autocorrelated, multivariate recruitment simulation; whether regime/ENSO robustness enters the current round or later pre-/post-SC work; the need for a clear Scientific Committee framing of climate effects; the outstanding anomalous F forecast; the lack of an F~MSY~ definition for the northern stock inside the CMP; the final reference-point proxy and averaging-window choice; and the total OM count and associated computational load.

# Draft set of Candidate Management Procedures (CMPs)

1. CMPs based on piecewise-linear harvest-control rules can be summarized by the threshold metric at which the rule begins to increase catch, the target catch reached above the trigger, and any inter-annual TAC-change constraints. This draft set was discussed on Day 3 and is included here as a placeholder for additional text. Part of the SCW17 process is for Members and participants to nominate CMPs for Scientific Committee consideration, after which redundant or materially overlapping CMPs should be eliminated so that the Scientific Committee can focus on a concise set of distinct alternatives. The table records the parameters used for the illustrative CMP set. The figure shows the static HCR shape for each row; smoothing choices and index selection affect the metric passed into the HCR and are therefore not shown directly in the curve.
...
#| tbl-cap: "Candidate piecewise-linear CMP parameters used for the illustrative HCR figure."
#| echo: false
#| message: false
#| warning: false
#| results: asis
...
#| fig-cap: 'Draft candidate management procedure piecewise-linear harvest-control-rule shapes. Output = "catch"; dashed vertical lines show trigger values, dotted vertical lines show limits, and horizontal lines show minimum and target catch.'
#| echo: false
#| message: false
#| warning: false
#| fig-width: 10
...
| Paper 06 | Evaluation of indices for CMP considerations |
