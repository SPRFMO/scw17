...
1. The composition of the OM reference set was debated. Including model 0.00 would add an optimistic high-biomass, low-exploitation outcome that could affect CMP tuning and communication. The Day 1 discussion focused on model variants already available to the group.

1. The practical Day 1 approach was to focus immediate tuning work on model 0.16, while retaining model 0.00 and the SC13-era configuration as clearly labelled alternatives or robustness tests. The final reference-set decision was identified for further discussion and Scientific Committee review.
...
1. The Day 1 decisions and agreements were to revise the agenda wording so the group would "collaboratively agree" rather than vote; use model 0.16 as the primary reference for OM conditioning during the workshop; retain model 0.00 and the SC13-era configuration as labelled alternatives or robustness tests; use the reduced-parameter, block-based Peruvian fleet selectivity formulation; accept the differentiable high-F penalty as a solver aid with biomass and recruitment estimates expected to remain effectively unchanged; keep the current data-simulation procedure at this stage; weight combined indices by representativity rather than catch; retain Kobe plots for stakeholder communication and add F~35%-based comparisons across OMs; tune CMPs across the reference set rather than to individual OMs; and submit the benchmark report, final Paper 01, and this workshop report to the Scientific Committee.
...
# Appendix B. Workshop Schedule {.unnumbered}
