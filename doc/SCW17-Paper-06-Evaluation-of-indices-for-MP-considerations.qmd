...
index_residual_data <- map_dfr(index_models, model_index_residual_df)
...
residual_correlation_matrix <- index_residual_data |>
  filter(index %in% covariance_indices) |>
...
@tbl-residual-summary gives the retained residual coverage by index. Residuals are the standardized index-fit residuals reported in the JJM `Obs_Survey_*` output for `h1_0.16`, after omitting `Chile_AcousCS` and dropping `Chile_AcousN` observations before 2006. @fig-residual-correlation-heatmap shows the Pearson correlations among those residuals for the same five retained indices used in the covariance simulation.

```{r}
#| label: tbl-residual-summary
#| tbl-cap: "Model fit residual coverage by index for h1_0.16. Residuals are standardized index-fit residuals from the JJM Obs_Survey output."
#| echo: false
...
```{r}
#| label: fig-residual-correlation-heatmap
