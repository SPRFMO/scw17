...
plot_qmd_ssb_dynamic_bzero <- function(worms_dt, om_short, n_iter, init_year,
  ...
  p <- ggplot(sum_dt, aes(x=year)) +
    ...
    zero_y_scale(labels=function(x) format(round(x, 2), trim=TRUE,
      scientific=FALSE)) +
    labs(title=paste("SSB / dynamic Bzero -", om_short), x=NULL,
      y="SSB / dynamic Bzero") +
    report_theme()
...
#| fig-cap: !expr paste0("Dynamic Bzero for ", REF_OM, " through ", FINAL_YEAR, ". The ribbon shows the 90% interval across MCMC draws, the black line shows the median, and colored lines show 15 sampled MCMC draws.")
#| fig-width: 12
#| fig-height: 6
