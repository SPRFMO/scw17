...
plot_qmd_dynamic_bzero <- function(om_short, n_iter, init_year, worm_n=15) {
  dat <- om_dynamic_bzero_dt(om_short, n_iter)[year <= init_year]
  sum_dt <- dat[, .(
  ...
  p <- ggplot(sum_dt, aes(x=year)) +
    ...
    zero_y_scale(labels=function(x) format(round(x), big.mark=",",
      scientific=FALSE)) +
    labs(title=paste("Historical dynamic Bzero -", om_short), x=NULL,
      y="Dynamic Bzero (t)") +
    report_theme()
...
The dynamic Bzero diagnostic shows the historical unfished spawning biomass
trajectory used to scale the terminal stock-status comparison.

```{r}
#| label: fig-dynamic-bzero
#| fig-cap: !expr paste0("Historical dynamic Bzero for ", REF_OM, " through ", INIT_YEAR, ". The ribbon shows the 90% interval across MCMC draws, the black line shows the median, and colored lines show 15 sampled MCMC draws.")
#| fig-width: 12
#| fig-height: 6
plot_qmd_dynamic_bzero(REF_OM, N_ITER, INIT_YEAR, worm_n=15)
```
