...
scenario_caption <- function() {
  ...
  paste("Forward projections under the simplified future fishing-pattern scenarios.",
...
plot_qmd_exploitation <- function(mets_dt, worms_dt=NULL, init_year, n_iter=NULL, ref_om=NULL) {
  ...
  if(!is.null(worms_dt) && nrow(worms_dt) > 0)
    sub <- paste0(sub, " | grey lines: ", length(unique(worms_dt$iter)), " MCMC draws")
  ...
  p <- ggplot(sum_dt, aes(x=year)) +
    ...
    { if(!is.null(worms_dt) && nrow(worms_dt) > 0)
        geom_line(data=worms_dt, aes(y=value, group=interaction(metric, stock, iter)),
          colour="grey45", alpha=0.38, linewidth=0.45) } +
...
plot_qmd_tradeoff <- function(status_dt, final_year, ref_om=NULL) {
  ...
  p <- ggplot(dat, aes(x=ssb_sbmsy, y=catch, colour=label, shape=label)) +
    geom_vline(xintercept=1, linetype="dotted", colour="#d55e00") +
    geom_segment(aes(x=ssb_lo, xend=ssb_hi, y=catch, yend=catch),
    ...
    labs(title=title, x="SSB / SBMSY", y="Catch (t)") +
    report_theme()
...
The simplified comparison uses future fishing patterns that include no fishing, constant future fishing mortality set to FMSY, constant future fishing mortality set to the 2025 value, constant future fishing mortality set to 75% and 50% of the 2025 value, constant fishing mortality set to the F35 SPR rate, and constant future catch set to half of the 2025 catch.
The catch-based scenario is allocated among fleets using the 2025 fleet catch proportions.
Special targets such as `FMSY`, `F2025`, `F2025_75`, `F2025_50`, `F35`, and `C2025_HALF` are resolved during the projection run before the cache is saved. `F35` is read from the model yield-profile output as the SPR rate that gives 35% of spawning biomass per recruit.
...
# Simplified Fishing Pattern Scenarios

The main projection comparison applies the simplified fishing-pattern controls over the projection horizon.
